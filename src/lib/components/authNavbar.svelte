<script lang="ts">
	...
	onMount(async () => {
		try {
			const res = await fetch(`${PUBLIC_API_URL}/api/users/me`, { credentials: 'include' });
			if (res.ok) {
	...
	function handleResultClick(item: Record<string, unknown>) {
		results = [];
		showResults = false;
		goto(resolve(`/listings/${item._id}`));
	}
...
<nav class="nav">
	...
	<div class="desktop-nav">
		<div class="items-wrapper">
			<div id="search-bar">
				...
				{#if showResults && results.length}
					<ul class="search-dropdown">
						{#each results as item (item._id as string)}
							<li>
			...
			<a href={resolve('/dashboard/listings/new')} class="sell-button">{$_('sell')}</a>
			<div class="user-area">
				<button on:click={toggle} class="notification-button" aria-label="Toggle notifications">
					{#if active}
						<svg class="notification-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"
							><path
				...
				<a
					href={resolve(username ? `/profile/${username}` : '/dashboard/listings')}
					class="user-profile-button"
					aria-label="View profile"
				>
					<svg class="user-profile-icon" xmlns="http://www.w3.org/2000/svg" viewBox="40 40 640 640"
						><path
		...
		<div class="nav-links">
			...
			<a href={resolve('/dashboard/listings')}>{$_('my_listings')}</a>
		</div>
	</div>
	...
	<div class="mobile-menu{mobileOpen ? ' open' : ''}">
		...
		<a href={resolve('/dashboard/listings')} class="mobile-link">{$_('my_listings')}</a>
		<a href={resolve('/dashboard/listings/new')} class="mobile-link">{$_('sell')}</a>
		<a
			href={resolve(username ? `/profile/${username}` : '/dashboard/listings')}
			class="mobile-link"
		>
