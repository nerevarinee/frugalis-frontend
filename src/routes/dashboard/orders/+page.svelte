<script>
  ...
  $: displayed = activeTab === 'pending'  ? pending
               : activeTab === 'accepted' ? accepted
               : rejected

  const deliveryIcon = { delivery: '', pickup: '' }
    ...
    <div class="order-list">
      {#each displayed as order (order._id)}
        <div class="order-card status-{order.status}">
          ...
          <!-- Listing info -->
          <a href="/listings/{order.listing?._id}" class="listing-preview">
            <img
              src={order.listing?.images?.[0] ?? '/placeholder.png'}
              alt={order.listing?.name}
            />
            <div>
              <p class="listing-title">{order.listing?.name ?? 'Deleted listing'}</p>
              <p class="listing-price">${order.listing?.price ?? '—'}</p>
            </div>
          </a>
          ...
          <div class="buyer-info">
            ...
            <div class="info-row">
              <span class="info-label">
                {deliveryIcon[order.deliveryType]} Method
              </span>
          ...
          <div class="actions">
            ...
            {:else if order.status === 'accepted'}
              <!-- Call buyer shortcut -->
              <a href="tel:{order.tel}" class="btn call">
                📞 Call Buyer
              </a>
            ...
            >
              ...
              <button
                ...
                on:click|preventDefault={(e) => {
                  if (confirm('Delete this order?')) e.target.closest('form').submit()
                }}
              >
                🗑
              </button>
